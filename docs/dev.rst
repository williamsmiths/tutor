...
For detailed explanations on how to work on elearning-edx and its dependencies, see the :ref:`edx_platform` tutorial.
...
Then, optionally, tell Tutor to use a local fork of elearning-edx::

    tutor mounts add ./elearning-edx
...
Additionally, when a local clone of elearning-edx is bind-mounted, it will:
...
.. note::
  ...
  If you've added your elearning-edx to the bind-mounted folders, you can remove at any time by running::

    tutor mounts remove ./elearning-edx
...
You can then import elearning-edx and django modules and execute python code.

To rebuild assets, you can run the ``build-dev`` NPM script that comes with elearning-edx::
...
- The user that runs inside the container has the same UID as the user on the host, to avoid permission problems inside mounted volumes (and in particular in the elearning-edx repository).
- Additional Python and system requirements are installed for convenient debugging: `ipython <https://ipython.org/>`__, `ipdb <https://pypi.org/project/ipdb/>`__, vim, telnet.
- The elearning-edx `development requirements <https://github.com/openedx/elearning-edx/blob/release/teak/requirements/edx/development.in>`__ are installed.
...
Values passed to ``tutor mounts add ...`` can take one of two forms. The first is explicit::
    ...
    tutor mounts add lms:/path/to/elearning-edx:/openedx/elearning-edx

The second is implicit::

    tutor mounts add /path/to/elearning-edx

With the explicit form, the value means "bind-mount the host folder /path/to/elearning-edx to /openedx/elearning-edx in the lms container at run time".

If you use the explicit format, you will quickly realise that you usually want to bind-mount folders in multiple containers at a time. For instance, you will want to bind-mount the elearning-edx repository in the "cms" container, but also the "lms-worker" and "cms-worker" containers. To do that, write instead::

    # each service is added to a coma-separated list
    tutor mounts add lms,cms,lms-worker,cms-worker:/path/to/elearning-edx:/openedx/elearning-edx

This command line is a bit cumbersome. In addition, with this explicit form, the elearning-edx repository will *not* be added to the build context at build time. But Tutor can be smart about bind-mounting folders to the right containers in the right place when you use the implicit form of the ``tutor mounts add`` command. For instance, the following implicit form can be used instead of the explicit form above::

    tutor mounts add /path/to/elearning-edx

With this implicit form, the elearning-edx repo will be bind-mounted in the containers at run time, just like with the explicit form. But in addition, the elearning-edx will also automatically be added to the Docker image at build time.
...
So, when should you *not* be using the implicit form? That would be when Tutor does not know where to bind-mount your host folders. For instance, if you wanted to bind-mount your elearning-edx virtual environment located in ``~/venvs/elearning-edx``, you should not write ``mounts add ~/venvs/elearning-edx``, because that folder would be mounted in a way that would override the elearning-edx repository in the container. Instead, you should write::

    tutor mounts add lms:~/venvs/elearning-edx:/openedx/venv

Verify the configuration with the ``list`` command::

    $ tutor mounts list
    - name: lms:~/venvs/elearning-edx:/openedx/venv
      build_mounts: []
      compose_mounts:
      - service: lms
        container_path: /openedx/venv

.. note:: Remember to setup your elearning-edx repository for development! See :ref:`edx_platform_dev_env`.
...
The above solution may not work for you if you already have an existing directory, outside of the "volumes/" directory, which you would like mounted in one of your containers. For instance, you may want to mount your copy of the `elearning-edx <https://github.com/openedx/elearning-edx/>`__ repository. In such cases, you can simply use the ``-v/--volume`` `Docker option <https://docs.docker.com/storage/volumes/#choose-the--v-or---mount-flag>`__::

    tutor dev run --volume=/path/to/elearning-edx:/openedx/elearning-edx lms bash

Override docker-compose volumes
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

.. warning:: While the option described below "works", it will only bind-mount directories at run-time. In many cases you really want to bind-mount directories at build-time. For instance: when working on elearning-edx requirements. As an alternative, you should consider following the instructions above: :ref:`persistent_mounts`.
...
You are then free to bind-mount any directory to any container. For instance, to mount your own elearning-edx fork::

    services:
      lms:
        volumes:
          - /path/to/elearning-edx:/openedx/elearning-edx
      cms:
        volumes:
          - /path/to/elearning-edx:/openedx/elearning-edx
      lms-worker:
...
This override file will be loaded when running any ``tutor dev ..`` command. The elearning-edx repo mounted at the specified path will be automatically mounted inside all LMS and CMS containers.
