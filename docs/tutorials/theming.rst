...
Comprehensive theming is enabled by default, but only the default theme is compiled. `Indigo <https://github.com/williamsmiths/indigo>`__ is a better, ready-to-run theme that you can start using today.
...
With Tutor, it's pretty easy to develop your own themes. Start by placing your files inside the ``env/build/openedx/themes`` directory. For instance, you could start from the ``red-theme`` theme present inside the ``elearning-edx`` repository::

    cp -r /path/to/elearning-edx/themes/red-theme "$(tutor config printroot)/env/build/openedx/themes/"
