...
- ``DOCKER_IMAGE_OPENEDX`` (default: ``"{{ DOCKER_REGISTRY }}williamsmiths/openedx:{{ TUTOR_VERSION }}"``)
...
- ``DOCKER_IMAGE_PERMISSIONS`` (default: ``"{{ DOCKER_REGISTRY }}williamsmiths/openedx-permissions:{{ TUTOR_VERSION }}"``)
...
By default, Tutor runs the `williamsmiths/openedx <https://hub.docker.com/r/williamsmiths/openedx/>`_ docker image from Docker Hub. If you have an account on `hub.docker.com <https://hub.docker.com>`_ or you have a private image registry, you can build your image and push it to your registry with::
