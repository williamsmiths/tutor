...
main() {
    ...
    echo ""
    echo "Bạn có thể cấu hình tutor để sử dụng image mới:"
    echo "tutor config save --set MFE_DOCKER_IMAGE=${TARGET_REGISTRY}/${IMAGE_NAME}:${new_version}"
    echo ""
    echo "Gợi ý cập nhật runtime sau khi đổi image (tuỳ chọn):"
    echo "docker stop tutor_local-mfe-1 || true"
    echo "docker rm -f tutor_local-mfe-1 || true"
    echo "docker rmi ${TARGET_REGISTRY}/${IMAGE_NAME}:${latest_version} || true"
...
if [ $# -eq 1 ]; then
    ...
    if [ $? -eq 0 ]; then
        ...
        if [ $? -eq 0 ]; then
            ...
            echo "docker stop tutor_local-mfe-1 || true"
            echo "docker rm -f tutor_local-mfe-1 || true"
            echo "docker rmi ${TARGET_REGISTRY}/${IMAGE_NAME}:${VERSION} || true"
            echo "tutor config save"
            echo "tutor local start -d"
        fi
    fi
